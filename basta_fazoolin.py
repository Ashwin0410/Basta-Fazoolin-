class Menu:
  ...
  def calculate_bill(self,purchased_items):
    ...
    for i in purchased_items:
      c+=self.items[i]
    return f"The total cost is {c} for the items {",".join(purchased_items)}"
...
class Franchise:
  def __init__(self,address,menus):
    ...
    self.menus = menus
  def __repr__(self):
    return f"The address of the franchise is {self.address}"
  def available_menus(self,time):
    return [i for i in self.menus if i.start_time<=time<=i.end_time]
